...
static float g_StopTime = 0.0f; // ボールが制止するまでの時間
...
void	Player2Initialize(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	...
	g_Player2.m_model = ModelLoad("asset\\model\\test.fbx");

	g_Player2.m_position = XMFLOAT3(0.0f, 0.5f, 1.0f);
	...
	//追加処理: 体力と武器の初期化
	g_Player2.m_MaxHealth = 100.0f; // 最大体力を100に設定
	g_Player2.m_Health = g_Player2.m_MaxHealth; // 現在の体力を最大体力に
	g_Player2.m_IsAttacking = false;

	// 武器の初期化（例としてCreateWeaponという関数を使用）
	// 実際には、他のcppで定義されている武器のインスタンスを生成し、ポインタをセットします。
	// g_Player2.m_CurrentWeapon = CreateWeapon(/* 武器のタイプ */);


	g_Player2.m_CurrentWeapon = WeaponFactory::CreateWeapon
	(
		WeaponType::SWORD, // 初期装備は剣
		pDevice,
		pContext
	);
	// m_AttackTimerの初期化をPlayer2.hへの追加を前提に追記
	g_Player2.m_AttackTimer = 0;
...
void	Player2Finalize()
{
	...
	{
		g_Player2.m_CurrentWeapon->Finalize();
...
void	Player2Update()
{
	...
	hal::dout << "--- Player2 Status ---\n";
	hal::dout << "Health: " << g_Player2.m_Health << "\n";
	if (g_Player2.m_CurrentWeapon)
	{
		hal::dout << "IsAttacking: " << (g_Player2.m_CurrentWeapon->IsAttacking() ? "TRUE" : "FALSE") << "\n";
	}
	hal::dout << "---------------------\n";
	...
	if (g_Player2.m_CurrentWeapon)
	{
		g_Player2.m_CurrentWeapon->Update(0.0f); // タイマーが進む
	}

	// 2. 武器が攻撃終了を要求しているかチェック
	if (g_Player2.m_CurrentWeapon && g_Player2.m_CurrentWeapon->IsAttacking())
	{
		if (g_Player2.m_CurrentWeapon->ShouldEndAttack()) // 剣自身が30フレームを超えたと判断
		{
			g_Player2.m_CurrentWeapon->EndAttack(); // 攻撃終了処理を実行
			// プレイヤー側の g_Player2.m_IsAttacking も武器側の IsAttacking() に置き換えることで
			// プレイヤー側のタイマーは完全に不要になります。
			hal::dout << "攻撃終了 (武器側タイマーで完了)\n";
			g_Player2.m_Health -= 50;
...
{
	// Game.hで宣言したGetPlayer2Controller()を使い、インスタンスを取得
	Controller* player2Controller = GetPlayer2Controller();

	// コントローラーが接続されているか確認 (IsConnected()は汎用チェック、UpdateState()はGame_Updateで呼ばれる前提)
	// 【修正】player2ControllerがNULLでなく、かつ接続されているかを確認
	bool isControllerConnected = player2Controller && player2Controller->IsConnected();

	// 【修正】デバッグコードを、接続されていれば入力値を出力するように変更
	if (isControllerConnected)
	{
		float thumbX = player2Controller->GetLeftThumbX();
		float thumbY = player2Controller->GetLeftThumbY();
		bool a_pressed = player2Controller->IsButtonPressed(XINPUT_GAMEPAD_A);
	...
	hal::dout << g_Player2.m_velocity.y << "\n";
	...
	// 横移動
	if (Keyboard_IsKeyDown(KK_A)) { strafe = +0.1f; }	// 左
	if (Keyboard_IsKeyDown(KK_D)) { strafe = -0.1f; }// 右
	...
	if (isControllerConnected)
	{
		float thumbX = player2Controller->GetLeftThumbX(); // X軸 (-1.0f ～ 1.0f)
		float thumbY = player2Controller->GetLeftThumbY(); // Y軸 (-1.0f ～ 1.0f)
...
// ここはキーボードとパッドの入力が合算された状態で処理される
	...
	//ジャンプ
	bool jumpInput = Keyboard_IsKeyDownTrigger(KK_SPACE);

	if (isControllerConnected)
	{
		// Bボタン (XINPUT_GAMEPAD_B) が押されているかをチェック
		jumpInput |= player2Controller->IsButtonPressed(XINPUT_GAMEPAD_B);
	}

	// スペース or Bボタンが押され、かつコヨーテタイムが残っている
	if (jumpInput && g_Player2.m_koyoteTime > 0.0f)
	{
	...
	//攻撃操作
	bool attackInput = Keyboard_IsKeyDownTrigger(KK_C);

	if (isControllerConnected)
	{
		// Aボタン (XINPUT_GAMEPAD_A) が押されているかをチェック
		attackInput |= player2Controller->IsButtonPressed(XINPUT_GAMEPAD_A);
	}
	...
	// 例として、cで攻撃を行う (キーボードのCキーは上記のattackInputに統合されているため、このブロックは基本的に不要)
	/*
	if (Keyboard_IsKeyDownTrigger(KK_C))
	{
		hal::dout << "攻撃した " << "\n";
		// 攻撃を開始する
		if (!g_Player2.m_IsAttacking)
		{

			g_Player2.m_IsAttacking = true;
			g_Player2.m_AttackTimer = 0; // タイマーリセット
			if (g_Player2.m_CurrentWeapon)
			{
				// プレイヤーの位置と回転を渡して攻撃開始
				g_Player2.m_CurrentWeapon->StartAttack(g_Player2.m_position, g_Player2.m_rotation);
			}
			// ここで武器の攻撃ロジック（例：武器モデルの描画、当たり判定生成）を呼び出す
			// g_Player2.m_CurrentWeapon->StartAttack(); 
		}
	}
	*/
	...
	// 別の場所（例：武器のUpdate関数やPlayer2Update内）で行う
...
void	Player2Draw()
{
	...
	if (g_Player2.m_CurrentWeapon)
	{
		g_Player2.m_CurrentWeapon->Draw(g_Player2.m_position, g_Player2.m_rotation);
	}
...
{
	// 体力を減らす
	g_Player2.m_Health -= damageAmount;

	// 体力が0以下にならないようにクランプ
	if (g_Player2.m_Health < 0.0f)
	{
		g_Player2.m_Health = 0.0f;
	}

	// デバッグ出力
	hal::dout << "Player2 Damaged! Current Health: " << g_Player2.m_Health << "\n";

	// 体力0になった場合の処理は Player2Update
}
