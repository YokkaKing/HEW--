...
{
    // 矢はチャージによって威力が変わるため、通常はHandleInputで発射されますが、
    // インターフェースを満たすため、ここではチャージなしの最小攻撃として発射します。
    ...
    // 発射実行
    ShootForward(playerPosition, playerRotation);

    // HandleInputがない場合に備えて、タイマーはリセットしない（HandleInputの後に呼ばれないことを期待）
}
// プレイヤーの向いている方向に矢を撃つ //追加
void Arrow::ShootForward(const XMFLOAT3& playerPosition, const XMFLOAT3& playerRotation) //追加
{
...
void Arrow::HandleInput(bool isAPressed, bool isAReleased, const XMFLOAT3& playerPos, const XMFLOAT3& playerRot) //追加
{
    ...
    if (isAReleased)
    {
        ...
        // プレイヤーの向きに矢を撃つ
        ShootForward(playerPos, playerRot);
